...
class MultipleChoiceController extends Controller
{
    public function index(Request $request){
        $getMultipleChoice = MultipleChoice::with('subject')->distinct()->get(['id','set_name','subject_id', 'is_activate']);
    ...
    public function editMcq(Request $request, $id){
        $mcq_id = \Crypt::decrypt($id);
        $mcq_data = MultipleChoice::where('id',$mcq_id)->where('is_activate',1)->get();

        return view('admin.multiple-choice.edit-mcq')->with(['mcq_data' => $mcq_data]);
    }
}
